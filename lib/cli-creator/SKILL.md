---
name: cli-creator
description: Build a composable CLI for agents from API docs, an OpenAPI spec, existing curl examples, an SDK, a web app, an admin tool, or a local script. Use when the user wants an agent to create a command-line tool that can run from any repo, expose composable read/write commands, return stable JSON, manage auth, and pair with a companion skill.
---
...
After the CLI works, create or update a small skill for it. Use `$skill-creator` when it is available. Prefer a project-local companion skill in the active agent's supported skills directory unless the user explicitly asks for a shared skill repo.
...
Keep API reference details in the CLI docs or a skill reference file. Keep the skill focused on ordering, safety, and examples future agent sessions should actually run.
