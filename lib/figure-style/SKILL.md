...
Nothing auto-loads it. Then call `apply_figure_style()` before plotting and use
helpers such as `focal_palette`, `bar_with_points`, `panel_letter`, and
`panel_crops` directly. Dependencies: `pip install matplotlib numpy pillow`;
`scipy` is optional for confidence intervals.
