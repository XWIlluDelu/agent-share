...
description: Implements Manus-style file-based planning to organize and track progress on complex tasks. Creates isolated .planning/<plan-id>/ directories containing task_plan.md, findings.md, and progress.md. Use when asked to plan out, break down, or organize a multi-step project, research task, or any work requiring 5+ tool calls. Supports automatic session recovery after /clear.
user-invocable: true
...
**Before doing anything else**, check whether an active `.planning` directory exists and read its planning files:

1. Resolve the active plan from `$PLAN_ID`, `.planning/.active_plan`, or the newest `.planning/<plan-id>/` directory. If found, read its `task_plan.md`, `progress.md`, and `findings.md` immediately.
...
- **Your planning files** go under **your project directory's `.planning/` directory** by default

| Location | What Goes There |
|----------|-----------------|
| Skill directory (`${CLAUDE_PLUGIN_ROOT}/`) | Templates, scripts, reference docs |
| `.planning/<plan-id>/` under your project directory | `task_plan.md`, `findings.md`, `progress.md` |

## Quick Start

Before ANY complex task:

1. **Create an isolated plan directory** — run `scripts/init-session.sh "Task Name"` so files are created under `.planning/<plan-id>/`
...
> **Note:** Planning files go under `.planning/<plan-id>/` by default, not the skill installation folder or project root.
...
|------|---------|----------------|
| `.planning/<plan-id>/task_plan.md` | Phases, progress, decisions | After each phase |
| `.planning/<plan-id>/findings.md` | Research, discoveries | After ANY discovery |
| `.planning/<plan-id>/progress.md` | Session log, test results | Throughout session |

## Critical Rules

### 1. Create Plan First
Never start a complex task without an active `.planning/<plan-id>/task_plan.md`. Non-negotiable.
...
- `scripts/init-session.sh` — Initialize planning files. Use a name arg by default to create an isolated plan under `.planning/YYYY-MM-DD-<slug>/` for parallel task workflows. Running without args writes root-level files only as legacy mode.
...
| Don't | Do Instead |
|-------|------------|
| Use TodoWrite for persistence | Create `.planning/<plan-id>/task_plan.md` |
| State goals once and forget | Re-read plan before decisions |
| Hide errors and retry silently | Log errors to plan file |
| Stuff everything in context | Store large content in files |
| Start executing immediately | Create plan file FIRST |
| Repeat failed actions | Track attempts, mutate approach |
| Create files in skill directory | Create files under `.planning/<plan-id>/` in your project |
