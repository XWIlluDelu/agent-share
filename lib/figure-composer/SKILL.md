...
```
Nothing auto-loads it. Then call the helpers
...
**Parallelize only if your platform has a sub-agent tool.** If your runtime supports sub-agents, you MAY dispatch one sub-agent per panel — each runs its `panel_task(outline,
