...
once per session by exec-ing this file in a Python cell (nothing auto-injects it):
...
def litrev_openalex_key_ok(key: str, timeout: float = 10) -> bool | None:
    ...
    req = urllib.request.Request(
        url, headers={"User-Agent": "AgentShare-literature-review/1.0"}
    )
    try:
...
def litrev_openalex_get(url: str, timeout: float = 15) -> dict | None:
    ...
    for attempt in (0, 1):
        req = urllib.request.Request(
            full, headers={"User-Agent": "AgentShare-literature-review/1.0"}
        )
        try:
            ...
            if e.code in (401, 409):
                raise RuntimeError(
                    ...
                    "under your OpenAlex credential configuration against "
            ...
            if e.code == 403:
                ...
                if litrev_openalex_key_ok(key, timeout) is False:
                    raise RuntimeError(
                        ...
                        "Re-check it in your OpenAlex credential configuration against "
                        "https://openalex.org/settings/api — do not retry "
                        "anonymously."
                    ) from None
                return None
...
def litrev_get(url: str, timeout: float = 15) -> dict | None:
    ...
    c = litrev_contact()
    ua = "AgentShare-literature-review/1.0" + (f" (mailto:{c})" if c else "")
...
def litrev_head(url: str, timeout: float = 10) -> int | None:
    ...
    c = litrev_contact()
    ua = ("AgentShare-literature-review/1.0" + (f" (mailto:{c})" if c else "")).encode(
        "ascii", "ignore"
    ).decode("ascii")
