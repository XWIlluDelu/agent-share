...
Show this to the user, then immediately proceed to Step 2. The user reads and thinks while the alternative designs are generated.

### 2. Generate independent designs

Generate 3+ independent designs, using parallel subagents if the active agent system supports them. Each design must produce a **radically different** interface for the deepened module.

Give each design path a separate technical brief (file paths, coupling details, dependency category from [DEEPENING.md](DEEPENING.md), what sits behind the seam). The brief is independent of the user-facing problem-space explanation in Step 1. Give each design path a different design constraint:
