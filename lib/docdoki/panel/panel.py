...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  .body.clamp{display:-webkit-box;-webkit-line-clamp:5;-webkit-box-orient:vertical;overflow:hidden;}
  ...
  .card [contenteditable]{cursor:text;}   /* card is a drag surface; its edit fields say so with a text caret */
  [contenteditable]:focus{outline:2px solid var(--link);outline-offset:1px;background:rgba(255,255,255,.7);}
  .more{display:none;border-top:2px solid #000;padding:9px 11px;background:#fff;}
  ...
  .kv .ev{outline:none;overflow-wrap:anywhere;}
  ...
  .node.c-done .body{background:var(--t-done);}
  .node.c-in-progress .body{background:var(--t-inprog);}
  .node.c-not-started .body{background:var(--t-notstarted);}
...
function cardHeight(id){if(measured[id])return measured[id];return open.has(id)?248:140;}
...
function layout(){
  ...
  const W=300,xG=150,topPad=46,leftPad=48;
...
(function(){const st=document.getElementById("stage");let pan=null;
  ...
  st.addEventListener("mousedown",e=>{
    ...
    if(e.target.isContentEditable||e.target.closest("button,.pill,.ttl,.toolbar"))return;
    const nd=e.target.closest("[data-node]");
    if(nd){const id=nd.dataset.node,od=dragOff[id]||{x:0,y:0};   // whole card drags; its controls opted out above
