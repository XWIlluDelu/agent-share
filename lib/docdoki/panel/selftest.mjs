...
check('Escape cancels only the active source session', () => {
  const s = new DraftStore(docs); s.set('A', 'A1'); s.begin('A'); s.update('A2'); s.end(true);
...
check('CRLF source sessions retain line endings', () => {
  const s = new DraftStore({ A: { source: 'A\r\nB\r\n' } }); s.begin('A'); s.update('A\nC\n'); s.end(); assert.equal(s.source('A'), 'A\r\nC\r\n');
});
...
if (process.argv.includes('--browser')) {
  ...
  try {
    ...
    await test('One-click document links and repeated/Unicode anchors survive editing; HTML and images stay inert', async () => {
      ...
      await page.locator('#reading a').filter({ hasText: 'Second' }).click(); assert.ok(await page.locator('#heading-repeated-1').isVisible());
      assert.equal(await page.locator('#reading img:not(.cm-widgetBuffer), #reading script').count(), 0); assert.equal(network.length, 0);
      await page.locator('#reading a').filter({ hasText: 'validation' }).click(); await page.waitForFunction(p => current === p, B);
      await open(); await page.locator('#reading a').filter({ hasText: 'reference' }).click(); await page.waitForFunction(p => current === p, B);
    });
    ...
    await test('Saving locks editor transactions, adopts disk sources and retains the follow receipt', async () => {
      ...
      try {
        ...
        assert.equal(await source(), await readFile(join(info.root, A), 'utf8')); assert.match(await page.evaluate(() => buildPrompt()), /already been saved/);
      } finally { await held.close(); }
      await page.evaluate(() => syncDocuments()); assert.match(await page.evaluate(() => buildPrompt()), /already been saved/);
    });
