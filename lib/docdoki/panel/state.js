...
class DraftStore {
  ...
  discard(path, latest) {
    if (this.busy || !this.base.has(path)) return false;
    if (latest) this.base.set(path, latest); else this.base.delete(path);
...
function diffHTML(before, after) {
  ...
  if (!ops) return `<p data-en="View the complete change below." data-zh="请查看下方完整修改。">View the complete change below.</p>${full.replace("<details", "<details open")}`;
