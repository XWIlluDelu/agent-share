...
document.addEventListener("click", async event => {
  ...
  if (edgeEl) {
    ...
    setHTML($("edge-detail"), `<button data-close-edge aria-label="${tr("Close", "关闭")}">×</button><p>${esc(catalogEntry(edge.from)?.title)} → ${esc(catalogEntry(edge.to)?.title)}</p><button data-remove-after="${esc(edge.stem)}" data-path="${esc(edge.to)}" ${store.busy ? "disabled" : ""}>${tr("Remove dependency", "移除依赖")}</button>`);
  }
});
