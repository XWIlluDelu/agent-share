...
async function changeDependency(path, stem, remove) {
  ...
  const previous = store, version = store.version;
  try {
    // Send intent against captured sources, never a list derived from a stale view.
    const result = await request("/preview", { edits: store.edits(), base: Object.fromEntries([...store.base].map(([p, d]) => [p, d.source])), after: { path, op: remove ? "remove" : "add", stem } });
    if (store !== previous || store.busy || version !== store.version) throw new Error(tr("Snapshot or draft changed while checking dependencies; change not applied. Retry.", "校验依赖时快照或草稿已变化，未应用修改，请重试。"));
    if (!result.ok) throw new Error(result.error);
    ...
    if (staged) changed(undefined, path);
    renderMain();
    status(staged ? tr("Dependency change staged, not saved.", "依赖修改已加入草稿，尚未保存。") : tr("Dependency already matches the request; no new change.", "依赖已符合请求，没有新增修改。"));
  } catch (error) { status(error.message, true); }
}
...
$("rebase").onclick = () => {
  ...
  finishEditing();
  store.rebase(latestDocument);
  latestDocument = null; $("comparison").hidden = true; changed();
