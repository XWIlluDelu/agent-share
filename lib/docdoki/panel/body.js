...
(() => {
  ...
  function headingKey(text, node) {
    function visible(node) {
      if (/^(HeaderMark|EmphasisMark|StrikethroughMark|CodeMark|LinkMark|URL|LinkTitle)$/.test(node.name)) return '';
      const value = text.slice(node.from, node.to);
      if (node.name === 'Escape') return value.slice(1);
      if (node.name === 'Entity') { const el = document.createElement('textarea'); el.innerHTML = value; return el.value; }
      if (!node.firstChild) return value;
      let out = '', at = node.from;
      for (let child = node.firstChild; child; child = child.nextSibling) {
        out += text.slice(at, child.from) + visible(child); at = child.to;
      }
  ...
  class EmptyCell extends WidgetType {
    constructor(header) { super(); this.header = header; }
    eq(other) { return this.header === other.header; }
    toDOM(view) {
      const cell = document.createElement('span'); cell.className = 'md-table-cell';
      cell.setAttribute('role', this.header ? 'columnheader' : 'cell');
      cell.addEventListener('mousedown', event => { event.preventDefault(); view.dispatch({ selection: { anchor: view.posAtDOM(cell) } }); view.focus(); });
      return cell;
    }
  ...
  function create(options) {
    ...
    function decorations(state) {
      const mode = state.field(display), cached = content(state), { text, start } = cached, ranges = [], lines = new Map(), headings = new Map();
      ...
      syntaxTree(state).iterate({ enter(node) {
        ...
        if (/^(?:ATX|Setext)Heading[1-6]$/.test(name)) {
          const level = name.at(-1), key = headingKey(text, node.node);
        ...
        if (name === 'Link' || name === 'Image' || name === 'Autolink') {
          ...
          if (url && marks.length >= 2) {
            const href = text.slice(url.from, url.to), resolved = options.resolveLink(href), attrs = { title: href, tabindex: '0', role: 'link', ...(!active ? { contenteditable: 'false' } : {}) };
            if (resolved?.external) Object.assign(attrs, { 'data-href': resolved.external });
            else if (resolved?.path) Object.assign(attrs, { 'data-href': '#' + encodeURIComponent(resolved.path), 'data-doc': resolved.path, 'data-anchor': resolved.anchor || '' });
            const labelFrom = name === 'Autolink' ? url.from : marks[0].to, labelTo = name === 'Autolink' ? url.to : marks[1].from;
            mark(labelFrom, labelTo, { tagName: 'span', class: resolved ? 'md-link' : 'unresolved', attributes: resolved ? attrs : { title: href } });
        ...
        if (name === 'Table') {
          for (let row = node.node.firstChild; row; row = row.nextSibling) {
            ...
            for (const [index, pipe] of pipes.entries()) {
              ...
              hide(pipe.from, pipe.to); edge = pipe.to;
            }
            if (edge < row.to && text.slice(edge, row.to).trim()) cells.push([edge, row.to]);
            for (const [a, b] of cells) {
              if (a === b) ranges.push(Decoration.widget({ widget: new EmptyCell(row.name === 'TableHeader'), side: 1 }).range(a));
              else mark(a, b, { class: 'md-table-cell', attributes: { role: row.name === 'TableHeader' ? 'columnheader' : 'cell' } });
            }
          }
        }
        if (name === 'ListMark') {
      ...
      if (!mode.source && (text.includes('[[') || text.includes(']['))) {
        ...
        for (const match of text.matchAll(/\[\[([^\]\n]+)\]\]|\[([^\]\n]+)\]\[([^\]\n]*)\]/g)) {
          ...
          const attrs = { title: target, tabindex: '0', role: 'link', 'data-href': resolved.external || '#' + encodeURIComponent(resolved.path), ...(!touched(from, to) ? { contenteditable: 'false' } : {}) };
    ...
    const extensions = [raw, display, markdown({ base: markdownLanguage, completeHTMLTags: false }),
      ...
      EditorView.domEventHandlers({
        ...
        pointerdown(event) {
          ...
          pointerLink = a ? { x: event.clientX, y: event.clientY, href: a.dataset.href, path: a.dataset.doc, anchor: a.dataset.anchor || '' } : null;
        },
        pointerup(event) {
        ...
        click(event) { if (event.target.closest('.md-link')) { event.preventDefault(); return true; } },
      }),
    ...
    const api = {
      ...
      setMode(source) {
        const mark = api.bookmark(), start = content(editor.state).start, selection = editor.state.selection.main;
      ...
      jump(anchor) {
        const { text, start } = content(editor.state), seen = new Map(); let target = null;
