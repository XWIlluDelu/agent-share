...
work identically on any agent that can run Python, with no
...
def pdf_resolve(path_or_vid):
    ...
    Artifact/version ids have no local store here — pass a real path to the PDF instead of an id.
    """
    ...
    if re.fullmatch(r"[0-9a-fA-F-]{32,36}", path_or_vid.strip()):
        raise FileNotFoundError(
            ...
            f"which has no local store here. Pass a "
...
def pdf_scan(path, query, top_k=5, mode="text", pages=None, dpi=100):
    ...
    relevance judgment. This replaces a parallel-LLM scan:
