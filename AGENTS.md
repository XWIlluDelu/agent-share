...
Act from the task's structure, not from the state you observe. Structure is the goal, the trust boundary, what is variable and what is constant, what counts as done. Observed state is the current code, the run layout, the incidental features of how a thing is today. Structure decides how observed state is read. Before acting, name the structure and derive the judgment from it; the absence of a guard in the code is not a methodological limit.
